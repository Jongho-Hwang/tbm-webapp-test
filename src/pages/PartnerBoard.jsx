...
export default function PartnerBoard() {
  ...
  const container = {
    padding:24,background:'#F5F5F7',minHeight:'calc(100vh - 64px)',
  };
  const buttonStyle = {
  ...
  const boardBox = {
    border:'2px solid #D1D1D6',borderRadius:16,padding:12,marginBottom:16,
  };
  ...
  useEffect(()=>{(async()=>{
    ...
    for(const n of ntcs){
      ...
      if(!(await getAck(n.id,me.uid))){
        await addAck({ id:ackId, noticeId:n.id, siteId:me.siteId,
                       partnerId:me.uid, ts:Date.now() });
      }
    }
  })();},[me.siteId,me.uid]);
  ...
  useEffect(()=>{(async()=>{
    ...
    if(recs.length){ setRecord(recs[0]); setTbmText(recs[0].text); setSubmitted(true);}
  })();},[me.uid,today]);
  ...
  const initRecognition = () => {
    ...
    if(!SR){alert('이 브라우저는 SpeechRecognition을 지원하지 않습니다.');return null;}
  ...
  const toggleRecording=()=>{
    if(!listening){
      if(!recognitionRef.current){const r=initRecognition();if(!r)return;recognitionRef.current=r;}
      recognitionRef.current.start(); setListening(true);
      window.speechSynthesis.speak(new SpeechSynthesisUtterance('녹음이 시작되었습니다'));
    }else{
      recognitionRef.current.stop(); setListening(false);
      window.speechSynthesis.cancel();
      window.speechSynthesis.speak(new SpeechSynthesisUtterance('녹음이 종료되었습니다'));
    }
  };
  ...
  const handleSave=async()=>{
    ...
    if(editMode){
      await updateTbmRecord(record.id,{text:tbmText}); setEditMode(false);
    }else{
      await saveTbmRecord({id:uuid(),siteId:me.siteId,partnerId:me.uid,
                           date:today,text:tbmText,createdAt:Date.now()});
      setSubmitted(true);
    }
    const recs = await fetchTbmByPartnerDate(me.uid,today); setRecord(recs[0]);
  };
  ...
  return(
    <div>
      ...
      <div style={container}>
        <h1 style={{fontSize:24,fontWeight:600,marginBottom:24}}>{me.uid} 관리자 대시보드</h1>

        {/* 녹음 */}
        <button
          style={{...buttonStyle,display:'flex',alignItems:'center',justifyContent:'center',gap:12,marginBottom:32}}
          onClick={toggleRecording}
        >
          <Mic size={28} color="#E03131" fill={listening?'#E03131':'none'}/>
        ...
        {/* 위험성 평가 교육 */}
        <section style={{marginBottom:40}}>
          <h2 style={{fontSize:20,fontWeight:500,marginBottom:12}}>위험성 평가 교육</h2>
          <div style={boardBox}>
            {educations.length?(
              <ul style={{listStyle:'none',padding:0,margin:0}}>
                {educations.map(e=>(
          ...
          <button style={{...buttonStyle,marginBottom:4}} onClick={()=>nav(`/board/${me.uid}/manage`)}>
        ...
        <section>
          <h2 style={{fontSize:20,fontWeight:500,marginBottom:12}}>
            {submitted&&!editMode?`제출된 TBM (${today})`:`오늘의 TBM (${today})`}
          </h2>

          {submitted&&!editMode?(
            <div style={{marginBottom:24}}>
              <div style={{whiteSpace:'pre-wrap',padding:12,background:'#FFF',
                           borderRadius:8,marginBottom:12,border:'1px solid #DDD'}}>
                {record&&record.text}
              </div>
              <button style={{...buttonStyle,marginBottom:12}} onClick={()=>setEditMode(true)}>수정하기</button>
            </div>
          ):(
            <div>
              <textarea
                style={{width:'100%',padding:12,border:'1px solid #CCC',borderRadius:8,
                        height:72,resize:'none',marginBottom:12}}
                placeholder="TBM 음성→텍스트"
                value={tbmText}
                onChange={e=>setTbmText(e.target.value)}
              />
              <div style={{display:'flex',gap:12,marginBottom:12}}>
                <button style={{...buttonStyle,flex:1}} onClick={handleSave}>
                  {editMode?'수정 완료':'제출하기'}
                </button>
                <button style={{...buttonStyle,flex:1}} onClick={()=>setShowQr(p=>!p)}>
                  근로자 페이지 QR코드 생성
                </button>
              </div>
              ...
              {showQr&&(
                <div style={{textAlign:'center',marginBottom:24}}>
                  ...
                  <p style={{fontSize:12,color:'#3C3C43'}}>스캔하여 근로자용 페이지 열기</p>
                </div>
              )}
            </div>
          )}
