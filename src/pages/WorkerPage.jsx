...
export default function WorkerPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [headSnap, siteSnap2, partnerSnap] = await Promise.all([
          getDocs(query(col, where('level', '==', 'head'))),
          getDocs(query(col,
            where('level', '==', 'site'),
        ...
        const list = [...headSnap.docs, ...siteSnap2.docs, ...partnerSnap.docs]
          .map(d => d.data());
        ...
        console.error('[WorkerPage] error', err);
      }
    })();
  }, [partnerId]);
  ...
  const siteName =
    (siteManager?.uid || partnerUser.siteId || '현장') + ' 현장';
